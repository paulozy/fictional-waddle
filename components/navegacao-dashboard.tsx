...
/**
 * Navegação do dashboard **abaixo de `md`**: a barra inferior fixa.
 *
 ...
 *
 * **Por que quatro abas e não seis.** São seis destinos, bem acima da faixa em
 * que uma barra inferior funciona (3–5). Mas eles não têm a mesma frequência:
 * agenda é diária, serviços e horários são de manutenção, fluxo é de
 * configuração inicial, WhatsApp só importa quando a conexão cai e conta é
 * quase nunca. Quatro abas dão ~82px cada num aparelho de 375px; seis dariam
 * ~55px e exigiriam rótulo de 9px. Os três destinos restantes, o tema e o Sair
 * moram no "Mais".
 *
 * Acima de `md` esta barra some por completo e quem navega é
 * `components/barra-lateral.tsx`, que mostra os mesmos destinos agrupados. Os
 * dois consomem `ICONES` e `ehAtivo` daqui, para não haver duas noções de
 * "página atual" nem dois mapas de ícone que possam divergir.
 */
 ...
 *
 * Exportada porque a barra lateral precisa da **mesma** noção de "página
 * atual": duas implementações divergiriam no primeiro destino com sub-rota.
 */
export function ehAtivo(caminho: string | null, href: string): boolean {
